...
This project is no longer actively maintained and considered deprecated.

Check out [[https://github.com/AndyFilter/YeetMouse][YeetMouse]] as a spiritual successor. It features everything that LEETMOUSE can do and so much more.

You can convert your existing LEETMOUSE config to a YeetMouse equivalent by simply running the =leet2yeet.py= script in this repo.
#+begin_src sh
# Method 1: New YeetMouse config style (March 2026)
./leet2yeet.py > yeetmouse.conf
# Copy that file to /etc/yeetmouse.conf after you installed YeetMouse including its 'yeetmousectl' binary.

# Method 2: Legacy YeetMouse config style (precompiled config)
./leet2yeet.py --legacy > default.h
# Copy the 'default.h' file to <YeetMouse repository>/driver/default.h and then follow the installation instructions of YeetMouse
#+end_src
...
* Description
  The =LEETMOUSE= kernelmodule enables quake-live like acceleration for your mouse on Linux and is heavily inspired by previous works from [[https://www.kovaak.com/mouse-acceleration][Povohat's mouse driver for Windows.]]
